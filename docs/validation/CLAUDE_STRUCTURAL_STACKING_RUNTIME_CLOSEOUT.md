...
# CLAUDE_GENERIC_STRUCTURAL_DISCOVERY_EIGHT_CONTROL_CLOSEOUT

**RESOLVED_STARTING_SHA:** `ddfceb3efc078fe144a8c872b6a0628f3707c5eb`

Scope: REG-5's own carried-forward cost-pool-aware pricing, plus multi-principal composition (HO-003/007/012/013, REG-4) and grant/selective-component wiring (HO-010/011).

## REG-5: RESOLVED (`canonical-1.78.0`)

The genuine cost-pool-aware re-pricing extension the prior pass deliberately deferred rather than rush. New `_try_cost_pool_aware_same_jurisdiction_stack()` in `canonical_evaluation.py`'s same-jurisdiction `location_groups` loop: identifies whichever program in a `same_cost_prohibited_distinct_costs_allowed` pair carries a genuine `CLOSED_POSITIVE_LIST` doctrine with real spend categories (`us_ny_post_production_credit`'s own registered `post_production`/`sound`/`vfx` `SpendRule`s -- never guessed from doctrine alone), derives the anchor's real per-line `AccountAllocation` via the same `derive_account_allocation()` every other candidate uses, partitions those real rows into two disjoint pools by each row's own real `spend_category` (a strict partition of one real tuple, so a `line_id` can never appear in both pools -- the same same-cost-refusal-by-construction principle `structural_archetype_generator.py` already uses), and prices each pool independently via the existing `price_segment()` kernel. Only persists `PRICED` when both pools independently clear their own program's real threshold/rate; otherwise persists a specific, reconstructable `RULE_REJECTED` (`COST_POOL_EMPTY` / `COST_POOL_MEMBER_UNPRICEABLE`).

Verified on `AUDIT_CONTROL_REG_5` (home=US-NY, $3,000,000 `atl_director` + $1,500,000 `post_production`): exact `PRICED` `{ny_state_film, us_ny_post_production_credit}` row, $1,425,000.00 total incentive (hand-calculated: $3,000,000 x 30% + $1,500,000 x 35%, matching the real, unmodified `RateRule`s exactly), $3,075,000.00 true net cost, disjoint line-ids conserving the full budget.

## REG-4 / multi-principal pairwise co-production: RESOLVED (`canonical-1.79.0`)

Confirmed via direct code reading that every existing combined-co-production pricing path (`_price_combined_coproduction_component_candidate`, both the home-anchored and non-home-anchored loops) unconditionally required a third movable-component target alongside the two treaty parties -- a pure 2-program co-production could never be reached even with a real, registered bilateral treaty and real, evidenced contribution facts on file. New `_price_combined_coproduction_pair_candidate()` is the direct sibling with the routed component omitted: the same real, evidenced (never invented) `majority_pct`/`minority_pct` treaty contribution facts become an explicit `spec.account_splits` entry across every non-memo account (`derive_account_allocation`'s own highest-precedence rule), reusing `price_allocated_structure` unchanged. Wired into the home-anchored bilateral loop immediately after the existing 3-way block, gated only on `RESOLUTION_ELIGIBLE`.

Verified on `AUDIT_CONTROL_REG_4` (home=GB, real GB-IE 80/20 `coproduction_majority_pct`/`minority_pct` `ProjectFact`s under the real, registered `uk-ie-bilateral` treaty -- `personnel_requirement=None` for this treaty, so no personnel facts were needed): exact `PRICED` `{ie_section_481, uk_avec}` row, $1,136,000.00 total incentive ($4,000,000 GB share x the real UK AVEC 80%-of-allocated-budget QPE cap x 25.5% net rate = $816,000.00, plus $1,000,000 IE share x Section 481's real flat 32% = $320,000.00), $3,864,000.00 true net cost -- hand-verified against the real, unmodified `RateRule`s. A second test confirms no `combined_coproduction_pair_stack` row is ever persisted with no evidenced contribution fact on file (never invents a split).

## HO-003 / HO-007: real capability confirmed, literal target not yet reproduced (honest partial)

Building `AUDIT_CONTROL_HO_003` (home=GB, real GB-AU 70/30 contribution facts, real director+writer GB-nationality `ProjectPerson`/`TalentProfile` rows satisfying `uk-au-bilateral`'s own real `personnel_requirement`) proved the pre-existing 3-way `combined_coproduction_component_stack` mechanism needs **zero new code** to fire with real facts -- multiple real `PRICED` `uk_avec`+`au_pdv_offset`+`<third program>` rows resulted (e.g. $1,804,600.00 total incentive / $5,695,400.00 NPC on a $7.5M budget). This is a genuine, previously-unverified capability confirmation. However HO-003's own literal named partner program, `au_producer_offset`, did not appear: `_best_priced_treaty_side_candidate()` selects the partner's own overall-best-priced program for that jurisdiction (`au_pdv_offset` outranked `au_producer_offset` for this fixture's real spend profile), not necessarily the treaty's own named `minority_unlocks` entry -- confirmed via direct code reading. Reproducing the exact literal combination needs either a fixture whose AU-eligible spend makes `au_producer_offset` AU's own best candidate, or a real, disclosed design decision (not attempted this pass, since it affects real production ranking behavior) on whether the helper should prefer a treaty's own named unlock.

HO-007 surfaced a genuine, real mismatch rather than a code gap: the real, registered `uk-fr-bilateral` treaty's own `minority_unlocks` are `['fr_tax_credit_cinema', 'fr_cnc_production']` -- never `fr_trip`. `fr_trip` is a real, separately-registered French program, but this codebase's own `structure_graph_model.py` associates it only with `ca-fr-bilateral`/`fr-de-bilateral`, never `uk-fr-bilateral`. HO-007's own required program set names a program the real UK-France treaty does not unlock.

Both dispositioned `MULTI_PRINCIPAL_PARTIALLY_RESOLVED` in the reconciliation CSV -- a new, honest status distinct from both "verified" and "architecturally deferred," since neither fully applies.

## HO-012 / HO-013: confirmed genuine architectural blockers (not forced)

HO-012 (three simultaneous principal legs, `fr_trip`+`ie_section_481`+`uk_avec`): `te.get_bilateral_treaty('IE','FR')` returns `None` -- no registered bilateral treaty between Ireland and France exists in `treaty_engine.py`'s own data, confirmed via direct query. Composing all three without a governing treaty or fact would require inventing a split, forbidden by this pass's own explicit constraint. Remains `MULTI_PRINCIPAL_DEFERRED`.

HO-013 (four simultaneous programs: 2 treaty parties + NZ post AND OCASE vfx together): confirmed via direct code reading that both the 3-way and the new pure-pair mechanisms route **at most one** movable component per combined structure (`for _combined_component, _combined_spend_amount in _combined_components:` persists one combined structure per component, never two components on the same structure). Genuinely unsupported shape, not an unresolved fixture. Remains `MULTI_PRINCIPAL_DEFERRED`.

## HO-010 / HO-011: refined finding, still genuinely deferred

`COMPONENT_BY_SPEND_CATEGORY` still has zero mappings to `fund_overlay`/`selective_upside` (unchanged, confirmed). New this pass: `app.calculators.conditional_programs.conditional_nodes_for(('CA-SK',))` already surfaces "Creative Saskatchewan Film and TV Production Grant" as a real, jurisdiction-matched disclosure on every CA-SK-touching structure via the already-wired `_conditional_data()` helper -- but under a different node identity (`COND-CA-SK-creative-saskatchewan-film-and-tv-production-grant`) than the program_slug `ca_sk_creative_saskatchewan_grant` HO-010 names, and as pure disclosure (no allocation, no `StructuralComponent`), not the shape HO-010's own direct-generator test requires. `conditional_nodes_for(('US-TN',))` does not surface `us_tn_performance_grant` or any Tennessee-specific node at all -- HO-011's root cause is more severe (no existing mechanism references this program by any identity). Reconciling the two catalogs (or building a real `fund_overlay` allocation basis) is a genuine, separate data-reconciliation task, deferred to the named follow-on workstream, not attempted this pass.

## Regression and acceptance evidence this pass

- `test_generic_structural_discovery_final_correction.py`: 16/16 (6 new tests: 3 REG-5 cost-pool tests, 3 REG-4 pairwise-coproduction tests).
- Broader regression: `test_structural_archetype_generator.py` + `test_ca_bc_dave_component.py` + `test_stacking_engine.py` + `test_canadian_labour_basis.py` + `test_ny_nm_or_validation.py` + `test_canada_validation.py` + `test_au_uk_copro_overview_wiring_claude.py` + `test_treaty_coproduction.py` + `test_treaty_coproduction_wiring.py` + `test_coproduction_optimizer_preservation.py`: 305/305.
- Fresh four-production batch (cache invalidated for all four real project IDs, evaluated once each under `canonical-1.79.0`): all `EVALUATION_COMPLETE`; every anchor incentive/NPC byte-identical to every prior pass (Little Utopia $573,059.70/$3,791,333.30; F#K Valentine's Day $1,445,659.84/$3,072,027.16; Bad Hombres $596,910.25/$1,885,112.75; Lips Like Sugar $3,459,278.90/$8,524,375.10); every `PRICED`/`DOMINATED_WITH_PROOF` count unchanged from the prior pass -- confirming neither new mechanism introduces any new economic candidate for a real production that has no real triggering fact on file (none of the four real productions carry same-jurisdiction distinct-cost-pool spend or treaty contribution `ProjectFact`s), exactly the expected, safe behavior.
- Semantic validator (`validate_claude_generic_structural_discovery.py --with-db`, isolated audit DB only): passes; three new statuses added to `VALID_STATUSES` (`PRICED_VERIFIED`, `MULTI_PRINCIPAL_PAIR_VERIFIED`, `MULTI_PRINCIPAL_PARTIALLY_RESOLVED`); exactly 19 rows confirmed.
- Shared `frametax2` database row count confirmed unchanged (533,788) before and after this pass.

## 19-control ledger, updated

`docs/validation/CLAUDE_GENERIC_DISCOVERY_19_CONTROL_RECONCILIATION.csv`: `DOMINATED_WITH_PROOF_VERIFIED` 4, `EXPECTED_RULE_REJECTION_EXACT_MATCH` 6, `NATURAL_EXACT_MATCH` 1, `PRICED_VERIFIED` 1 (REG-5), `MULTI_PRINCIPAL_PAIR_VERIFIED` 1 (REG-4), `MULTI_PRINCIPAL_PARTIALLY_RESOLVED` 2 (HO-003, HO-007), `MULTI_PRINCIPAL_DEFERRED` 2 (HO-012, HO-013), `GRANT_COMPONENT_UNWIRED_DEFERRED` 2 (HO-010, HO-011). Total exactly 19, all statuses mutually exclusive, validator-enforced. 13 of 19 controls now carry a genuine `evaluate_project()`-verified disposition (up from 11); the remaining 6 carry a real, precisely-evidenced finding each, never a blanket "deferred."
...
# CLAUDE_GENERIC_STRUCTURAL_DISCOVERY_STRUCTURAL_OPTIMIZER_WIRING_CORRECTION

**RESOLVED_STARTING_SHA:** `64e72b971d2af6dc0b9c3d0a2d7521847679c30b`
**Date:** 2026-09-17

Scope: a focused correction pass over the six-control closeout above, resumed at `ba76cd2f` per explicit instruction. The six-control closeout's own acceptance criteria were not fully met: HO-003's evidence cited a non-literal example instead of the required target; HO-012 priced an unsupported "national treatment" assumption; HO-013 used an arbitrary, component-agnostic search bound; HO-011's fix was a consumer-side workaround rather than a source-level fix; and neither HO-010 nor HO-011's FULL required 3-program controls (not just their standalone components) had been genuinely tested.

## HO-003: no code change needed -- evidence corrected

Direct re-verification found the control's own EXACT LITERAL required target ({uk_avec, au_producer_offset, NZ's international-post-vfx grant}) already reaches an exact `PRICED` match on `AUDIT_CONTROL_HO_003`: $1,939,600.00 total incentive / $5,560,400.00 true net cost. The six-control closeout's own CSV evidence had cited a different, non-literal illustrative example (`au_producer_offset` + `uz_film_rebate`) instead of testing the literal target -- a documentation defect, not a code gap. Corrected in the ledger and the prevention test (`test_ho003_literal_required_target_reaches_exact_priced_match`).

## HO-013: REVISED (`canonical-1.81.0`) -- arbitrary bound removed, literal target confirmed real-but-dominated

The six-control closeout's `_MULTI_COMPONENT_TARGET_BOUND=200` sliced the SAME global, component-AGNOSTIC `_combined_top_targets` list for every routed component -- a genuinely arbitrary cutoff, not a proof, that could silently exclude a component-specific real candidate ranked below 200 globally even while ranking near the top of its OWN component's real list. Removed entirely, replaced with (1) `_hy_component_all_targets[component]` -- the SAME real, independently-priced-per-component list the pre-existing `ordinary_component_hybrid` mechanism already builds -- and (2) a genuine pigeonhole exchange proof-based widening search (window starts at 2, doubles on failure), mirroring `canonical-1.72.0`'s own established proof pattern. Direct verification confirms both of the control's own literal components (NZ's post/vfx grant, OCASE) are real, independently-priced candidates for this exact fixture, but neither wins against a real Canadian-dominance combo (`ca_mb_film_video_credit`+`ca_nl_all_spend_credit`) -- the SAME pattern already established and accepted for HO-001/002/008/009. Reclassified `PRICED_VERIFIED` -> `DOMINATED_WITH_PROOF_VERIFIED` to match. Runtime dropped from ~36s to ~3s as a direct, disclosed consequence of using real, small, component-scoped lists instead of an arbitrary flat 200-candidate slice.

## HO-012: REVERTED (`canonical-1.81.0`) -- insufficient primary authority to price

Direct primary-authority verification found the six-control closeout's Eurimages pricing basis unsupported: `treaty_engine.py`'s own `eurimages-multilateral` `TreatyData` carries EMPTY `majority_unlocks`/`minority_unlocks` (this codebase's own structured, authoritative "this treaty unlocks these specific programs" fields, populated with real slugs for every bilateral treaty) -- only `fund_unlocks=["eu_eurimages"]` (the fund itself, not each party's own program) is populated. The sole textual support was an uncited free-text `notes` field (`confidence_tier="PARSED"`, no citation field -- `TreatyData` has no citation field at all, unlike `PersonnelRequirement.citation`, which does exist and is populated elsewhere in this file for individually-researched propositions). Per explicit instruction not to treat fund membership as national-treatment authority without primary proof, this branch now persists an explicit `RULE_REJECTED`/`MULTILATERAL_NATIONAL_TREATMENT_UNVERIFIED` for every structurally-eligible subset instead of pricing. Real subset-eligibility discovery (participant count, per-party minimum contribution share, cultural test) remains intact and disclosed. Reclassified `PRICED_VERIFIED` -> `RULE_REJECTED_VERIFIED`.

## HO-011: root cause corrected at its SOURCE (`AUTHORITY_COVERAGE_REGISTRY_VERSION` 1.8.0 -> 1.9.0)

The six-control closeout's fix was a consumer-side workaround in `_capability_only_status()` that special-cased `us_tn_performance_grant` by checking the older `_B1_DISCRETIONARY_RULING` registry when the newer `COVERAGE_REGISTRY` disagreed. Direct investigation found the TRUE cause: `program_rate_rules_worldwide.py`'s `US_TN_DOCTRINE` carries a real, `VERIFIED`-tier, officially-cited (tn.gov, fetched directly) `RateRule` -- a single unconditional flat-25%-of-QPE tier (min $200,000 QPE, no annual cap, zero `RateCondition`s) -- structurally identical in kind to 18 programs this SAME file's own changelog already documents removing from `_B1_DISCRETIONARY_RULING` as "genuinely misclassified as authority-exhausted"; `us_tn_performance_grant` was evidently missed by that pass. Removed from `_B1_DISCRETIONARY_RULING` this pass, and the consumer-side workaround is removed in the same pass (reverted to its original, simpler form), since the disagreement it existed to disclose no longer exists -- ONE canonical determination now governs. `us_tn_performance_grant` now prices normally: $750,000.00 (=25% of $3,000,000 QPE) standalone, and genuinely participates in the FULL discovery pipeline -- `AUDIT_CONTROL_HO_011_FULL` (mirroring HO-001's own real fixture exactly) confirms a real `DOMINATED_WITH_PROOF` (window=2, dominated_combination_count=109, incumbent=`ca_mb_film_video_credit`+`ca_nl_all_spend_credit`+`us_ga_film_credit`, best_real_total_found_usd=$291,053.50) for the {post,vfx} subset under the US-GA anchor -- the SAME Canadian-dominance pattern as HO-001, with NZ's grant and Tennessee's grant both real, examined, non-suppressed candidates. Reclassified `AUTHORITY_INSUFFICIENT_VERIFIED` -> `DOMINATED_WITH_PROOF_VERIFIED`.

## HO-010: status corrected to accurately reflect scope -- component-blocked, not canonically executed

The standalone Creative Saskatchewan identity reconciliation (`canonical-1.80.0`) remains correct: `conditional_programs.py`'s catalog node now carries `canonical_program_slug == 'ca_sk_creative_saskatchewan_grant'`, visible on a real persisted structure. But the control's own `required_program_set` names THREE programs together (`ca_federal_cptc`|`ca_sk_creative_saskatchewan_grant`|`on_ofttc`), not one. Direct verification via `economic_block_for_program('ca_sk_creative_saskatchewan_grant')` confirms BOTH authority registries independently classify it `DISPLAY_ONLY_ZERO_GUARANTEED` -- a real, CONFIRMED discretionary award, not a data gap -- so it never enters `priced_by_code` and can never structurally combine with `ca_federal_cptc`/`on_ofttc` into ONE combined `PRICED`/`RULE_REJECTED` structure. Per the explicit instruction to either reach a full terminal disposition or accurately label the control component-blocked, `AUTHORITY_INSUFFICIENT_VERIFIED` (which overclaimed full-control verification) is corrected to the new `COMPONENT_BLOCKED_NOT_CANONICALLY_EXECUTED` status -- an honest label distinct from both "verified" and "deferred", since the blocking reason IS real, confirmed, and fully disclosed, just architecturally incapable of combining into one structure without a separate `fund_overlay` component-wiring engineering effort (not attempted, per explicit no-broadened-scope instruction).

## Audit for remaining arbitrary caps (item 6)

Searched every function touched by the six-control closeout for other fixed top-N/search caps. `_all_priced_treaty_side_candidates`, `_real_multilateral_subset_participants`, `_price_combined_coproduction_multi_component_candidate`, and `_price_combined_multilateral_coproduction_candidate` carry none. The pre-existing `MAX_STACK_GROUP_SIZE`/`MAX_EURIMAGES_DISPLAY` constants are outside this pass's touched code (unrelated, long-standing mechanisms -- the latter is explicitly a DISPLAY-only bound per its own comment, not an evaluation bound) and were left untouched.

## Regression and acceptance evidence this pass

- `test_generic_structural_discovery_final_correction.py`: 25/25 (all six-control-closeout tests updated/replaced to match the corrected behavior, plus new tests for the registry source-fix and the component-blocked characterization).
- Consolidated regression (11 files spanning structural archetypes, Canadian labour/validation, stacking, and treaty co-production): see final report for the exact count.
- Fresh four-production batch under `canonical-1.81.0`: see final report.
- Semantic validator (`validate_claude_generic_structural_discovery.py --with-db`, isolated audit DB only): passes non-vacuously; `COMPONENT_BLOCKED_NOT_CANONICALLY_EXECUTED` added to `VALID_STATUSES`; `AUTHORITY_INSUFFICIENT_VERIFIED` retired entirely; exactly 19 rows confirmed, zero unresolved.

## 19-control ledger, final

`DOMINATED_WITH_PROOF_VERIFIED` 6 (HO-001, HO-002, HO-008, HO-009, HO-011, HO-013), `EXPECTED_RULE_REJECTION_EXACT_MATCH` 6, `NATURAL_EXACT_MATCH` 1, `PRICED_VERIFIED` 2 (REG-5, HO-003), `MULTI_PRINCIPAL_PAIR_VERIFIED` 1 (REG-4), `RULE_REJECTED_VERIFIED` 2 (HO-007, HO-012), `COMPONENT_BLOCKED_NOT_CANONICALLY_EXECUTED` 1 (HO-010). Total exactly 19, all statuses terminal, validator-enforced. `required_program_set` unchanged for every control this pass touched (no target was redefined to match whatever the code happened to produce).
