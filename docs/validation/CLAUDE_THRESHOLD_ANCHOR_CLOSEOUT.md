...
The row-19 table's `DISCOVERY_STAGE_AMOUNT_NOT_PROBED` disposition for `au_location_offset`/`ca_bc_dave`/`ma_ccm_rebate`/`th_film_incentive`, and this closeout's own characterization of the gap as "a distinct, larger architecture change... explicitly out of scope for this narrow workstream (not built this pass)," is **superseded**. The generic amount-fact-probing mechanism described there as future work was built in the following workstream: `discover_executable_jurisdictions()` and `_price_candidate`'s preflight now generically probe every executable program's `amount_fact_key` conditions from `qpe_usd` via the canonical FX path, and `price_segment` derives the real, tight, allocation-based amount at pricing time. `ca_bc_dave` and `th_film_incentive` now genuinely price for all 4 real productions; `au_location_offset` and `ma_ccm_rebate` now reach real candidate construction and correctly report a genuine, disclosed threshold shortfall (not a phantom discovery-stage block). See `CLAUDE_AMOUNT_GATED_DISCOVERY_REPAIR.csv`, `CLAUDE_AMOUNT_GATED_RUNTIME_PROOF.csv`, and `CLAUDE_AMOUNT_DISCOVERY_REPAIR_CLOSEOUT.md` for full detail.
