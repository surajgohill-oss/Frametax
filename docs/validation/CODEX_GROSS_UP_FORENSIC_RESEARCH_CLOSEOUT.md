# Codex Forensic Gross-Up Practice and Authority Research Closeout

Status: `GROSS_UP_FORENSIC_RESEARCH_COMPLETE_WITH_DOCUMENTED_AUTHORITY_GAPS`

## Scope and reconciliation

- Canonical unique economic program identities crosswalked: **658 / 658**.
- Physical coverage rows preserved in the source ledger: **659**.
- Existing settled authority records unambiguously recovered: **614**; unmatched identities remain explicit documented gaps rather than inferred permission.
- Transaction archetypes independently classified: **11 / 11**.
- Four-project forensic matrix rows: **116** (29 current authority-cap rules × 4 projects).
- Production code changed: **none**. Production database changed: **none**.

## Ireland worked lead case
...
- **Authority-confirmed:** genuine substantiated Ireland producer services/fee; arm’s-length pricing; actual-net-cost adjustment; exclusion of backend compensation.
...
Changing-denominator producer-fee ceilings use `x = max(0, (p × B − C) / (1 − p))`, not `p × B − C`. The matrix shows conditional 10% and 15% Ireland scenarios separately. Every row labels headroom as theoretical. **Recognized incremental QPE is not established**, not silently set to zero. Immediate cash, payable/equity and ultimate obligations remain separate. Permanent NPC benefit is `UNKNOWN_NOT_AUTHORIZED` until a real transaction, qualification, payment, assistance treatment, and any required ruling exist.
...
These artifacts are research and reconciliation inputs only. They do not change the canonical runtime or authorize automatic gross-up. Cap headroom is not QPE; deferred or loan obligations are not free support; equity is a separate financing flow; no recursive credit-on-credit treatment is authorized.
