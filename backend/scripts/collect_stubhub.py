...
_FETCH_TIMEOUT_SECS  = 300   # overall timeout per event (increased to allow 500+ listings)
...
async def fetch_stubhub_listings(
    ...
    _deadline = _time.monotonic() + _FETCH_TIMEOUT_SECS - 20  # reserve 20s for final extraction
    ...
    async with async_playwright() as p:
        ...
        for _ in range(_MAX_SHOW_MORE_CLICKS):
            # Stop clicking if we're approaching the overall deadline —
            # reserve time for the final DOM extraction and posting.
            if _time.monotonic() >= _deadline:
                if verbose:
                    print(f"[stubhub] approaching deadline — stopping Show-more loop to extract partial results")
                break
        ...
        # Extract all listing cards (runs even if loop exited early due to deadline)
...
def parse_listings(raw_listings: list) -> Tuple[list, dict]:
    ...
    for item in raw_listings:
        ...
        listings.append({
            ...
            "row":                 (str(item.get("row"))[:20] if item.get("row") else None),
