...
async def list_venues(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Venue).options(selectinload(Venue.sections)).order_by(Venue.name)
    )
...
async def get_venue_sections(slug: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Venue).options(selectinload(Venue.sections)).where(Venue.slug == slug)
    )
        ...
        "sections": [
            {
                ...
                "height": s.height,
                "shape": s.shape,
                "stubhub_aliases": s.stubhub_aliases,
                "seatgeek_aliases": s.seatgeek_aliases,
            }
            for s in v.sections
        ],
    }
