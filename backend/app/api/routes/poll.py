...
async def trigger_poll(
    ...
    result = await db.execute(
        select(TrackedEvent).where(TrackedEvent.event_id == event_id, TrackedEvent.is_active == True)
    )
    tracked = result.scalars().all()
    if not tracked:
        raise HTTPException(404, "No active tracked events")
    for te in tracked:
