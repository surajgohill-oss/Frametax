...
Fetch strategy:
  1. Official API (api.seatgeek.com/2/listings) — requires client_id
  2. Internal undocumented API endpoint
  3. __NEXT_DATA__ JSON embedded in page HTML
  4. Playwright with response interception (if all above fail)

Known failure patterns stored in FailureMemory:
  - __NEXT_DATA__ key path changes with frontend deploys
  - Internal API endpoint path changes
"""
...
NEXTDATA_KEY_PATHS = [
    # Try multiple known paths for __NEXT_DATA__ — checked in order
...
class SeatGeekCollector(BaseCollector):
    ...
    async def _fetch_listings(self, tracked_event) -> list[RawListing]:
        ...
        if not event_id:
            raise ValueError("No SeatGeek event ID — add event ID or URL")

        if self.debug_mode:
            self._debug_log(f"Resolved event_id={event_id}")

        # Try official API first if client_id available
        if self._client_id:
        ...
        # Internal API
        skip_internal = await self.should_skip_pattern(SEATGEEK_INTERNAL_API, "http_failure")
        if not skip_internal:
            listings = await self._fetch_internal_api(event_id)
            if listings is not None:
                return listings

        # __NEXT_DATA__ from page HTML
        ...
        # Playwright fallback
        if self.debug_mode:
            self._debug_log("All HTTP paths failed — falling back to Playwright")
    ...
    async def _fetch_official_api(self, event_id: str) -> Optional[list[RawListing]]:
        ...
        try:
            ...
            if self.debug_mode:
                self._debug_log(f"Official API HTTP {resp.status_code}")
    ...
    async def _fetch_internal_api(self, event_id: str) -> Optional[list[RawListing]]:
        ...
        try:
            ...
            self.logger.warning("SeatGeek internal API failed: %s", e)
            return None

    async def _fetch_nextdata(self, url: str, event_id: str) -> Optional[list[RawListing]]:
        """Extract listings from embedded __NEXT_DATA__ JSON."""
        client = await self._get_http_client()
        try:
            ...
            # Try each known key path, consulting FailureMemory for skips
            for path in NEXTDATA_KEY_PATHS:
                ...
                try:
                    ...
                    if listings:
                        ...
                        if self.debug_mode:
                            self._debug_log(f"__NEXT_DATA__ path '{path_key}' worked: {len(listings)} listings")
                        return listings
            ...
            self.logger.warning("SeatGeek __NEXT_DATA__ failed: %s", e)
            return None
    ...
    async def _fetch_via_playwright(self, url: str) -> list[RawListing]:
        ...
        async with async_playwright() as p:
            ...
            await self._debug_pause("SeatGeek page loaded")

            if self.debug_mode:
                await self._capture_screenshot("seatgeek_result")
        ...
        for data in captured:
            listings.extend(self._parse_api_response(data))
        return listings
    ...
    def _parse_api_response(self, data: dict) -> list[RawListing]:
        ...
        for item in data.get("listings", []):
            try:
                ...
                fees = None
                if fees_raw:
                    fees = Decimal(str(fees_raw.get("amount", 0) if isinstance(fees_raw, dict) else fees_raw))
                listings.append(RawListing(
                    ...
                    price=price,
                    fees=fees,
                    all_in_price=(price + fees) if fees else None,
                    listing_url=item.get("listing_url"),
                ))
            except Exception as e:
                self.logger.debug("Skip malformed listing: %s", e)
        return listings
    ...
    async def _extract_event_id(self, url: str) -> Optional[str]:
        ...
        try:
            client = await self._get_http_client()
            resp = await client.get(url)
            for pattern in [r'"event_id"\s*:\s*(\d+)', r'"id"\s*:\s*(\d+)']:
