...
async def test_fvd_accounting_matches_codex_diagnosis(db: AsyncSession):
    ...
    assert accounting["review_required_count"] == 57
    assert accounting["unpriceable_count"] == 52
    ...
    assert len(review_ranked) == 57
    assert len(unpriceable_ranked) == 52
