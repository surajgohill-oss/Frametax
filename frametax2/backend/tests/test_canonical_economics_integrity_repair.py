...
def test_dollar_cap_resolver_prefers_the_smallest_applicable_cap():
    ...
    cap, kind, basis, _fx_err, _unresolved = _resolve_incentive_dollar_cap("cy_film_rebate")
    ...
    cap, kind, basis, _fx_err, _unresolved = _resolve_incentive_dollar_cap("gr_cash_rebate")
...
def test_no_priced_segment_ever_exceeds_its_own_declared_dollar_cap():
    ...
    for slug in _RULES_BY_PROGRAM:
        if blocks_economic_candidacy(slug):
            continue
        cap, _, _, _fx_err, _unresolved = _resolve_incentive_dollar_cap(slug)
        if not cap:
            continue
