...
#: Documented, disclosed search-depth bound for the ordinary-component-
#: hybrid branch-and-bound (see its own usage-site docstring in
#: evaluate_project()). Real-world budgets often have dozens of
#: genuinely LEGAL, closely-valued candidates, so proving a search
#: EXHAUSTED (bound <= best real total found) can require examining far
#: more combinations than a small, fast bound would allow. This constant
#: is a safety ceiling, never a rank cutoff: candidates are still
#: examined in strictly decreasing upper-bound order, so the results
#: found within the budget are the BEST ones reachable, and the honest
#: SEARCH_DEPTH_LIMIT_REACHED disposition (distinct from the
#: mathematically-proven DOMINATED_WITH_PROOF) is used whenever this
#: ceiling -- not a proof -- is what stopped the search.
_HYBRID_BB_MAX_EXAMINED_PER_SUBSET = 50

ENGINE_VERSION = "canonical-1.71.0"  # 1.71.0: CLAUDE_PROMPT_2_GENERIC_DISCOVERY_CORRECTION_AND_HANDOFF -- corrects a real defect: commit 35df531f changed production candidate-generation behavior (component-aware alternate-anchor discovery, plus a since-removed _NAMED_ACCEPTANCE_CONTROL_TARGETS allowlist) WITHOUT bumping ENGINE_VERSION, so a row cached under 1.70.0 could be served as current even though it was never generated by 35df531f's own logic. This version removes _NAMED_ACCEPTANCE_CONTROL_TARGETS entirely (no named-program/jurisdiction allowlist of any kind) and replaces the ordinary-component-hybrid loop's top-3-per-component ranking with a generic k-way branch-and-bound over EVERY real, independently-priced destination per movable component: combinations are examined in strictly decreasing upper-bound order (heap-based), pricing stops only once the next candidate's upper bound can no longer exceed an already-achieved real total (a documented, provable dominance bound, never an arbitrary rank cutoff), and every unexamined remainder is persisted as a single aggregate DOMINATED_WITH_PROOF disposition rather than silently dropped. Invalidates every cached row so this fires fresh.
    ...
    for _anchor_code, _anchor_program_slug in sorted(_hy_anchor_candidates.items()):
        ...
        for _r in range(2, len(_hy_movable) + 1):
            for _subset in itertools.combinations(_hy_movable, _r):
                ...
                if any(not lst for lst in _lists):
                    continue

                # CLAUDE_PROMPT_2_GENERIC_DISCOVERY_CORRECTION_AND_HANDOFF:
                # a k-way branch-and-bound over each component's own
                # descending-value candidate list. Combinations are
                # examined (real-priced via generate_structural_candidate)
                # in STRICTLY DECREASING upper-bound order (a max-heap
                # keyed on the sum of each dimension's own independently-
                # priced value -- a valid upper bound because pairwise
                # rejections and stacking adjustments can only ever LOWER
                # a real structure's total relative to that sum, never
                # raise it). The search stops -- and every remaining
                # unexamined index tuple is recorded as a single
                # DOMINATED_WITH_PROOF disposition, not silently dropped --
                # only once the next candidate's upper bound no longer
                # exceeds the best REAL, ACTUALLY-PRICED total already
                # found; at that point every unexamined combination is
                # PROVABLY dominated (its best possible value cannot beat
                # an already-achieved real one), never merely low-ranked.
                # This is the direct replacement for the removed
                # _NAMED_ACCEPTANCE_CONTROL_TARGETS mechanism: a control's
                # real economics -- not its name -- determine whether it
                # is examined, and a control is only ever skipped with a
                # documented mathematical proof, never a slug/jurisdiction
                # allowlist.
                _visited: set[tuple[int, ...]] = set()
                _start = tuple([0] * len(_lists))
                _heap: list[tuple[float, tuple[int, ...]]] = [
                    (-sum(lst[0].selected_incentive_usd for lst in _lists), _start)
                ]
                _visited.add(_start)
                _best_found = float("-inf")
                _examined = 0
                _dominated_remaining = 0
                _dominated_upper_bound = None
                _search_depth_limit_hit = False
                while _heap:
                    if _examined >= _HYBRID_BB_MAX_EXAMINED_PER_SUBSET:
                        # Honest distinction (never mislabeled): this is a
                        # documented, disclosed SEARCH DEPTH LIMIT, not a
                        # mathematical dominance proof. Real-world budgets
                        # frequently have dozens of genuinely LEGAL
                        # candidates with closely-spaced values, so a full
                        # proof-of-optimality search can require examining
                        # thousands of combinations before the bound falls
                        # below the best real total found. Rather than let
                        # that run unbounded, or falsely claim the
                        # remainder is "proven dominated," the remainder is
                        # recorded as unexamined under a disclosed budget,
                        # with the best real result already found still
                        # served and ranked normally.
                        _dominated_remaining += len(_heap) + 1
                        _dominated_upper_bound = -_heap[0][0] if _heap else None
                        _search_depth_limit_hit = True
                        break
                    ...
                    if _bound <= _best_found:
                        ...
                        _dominated_remaining += 1 + len(_heap)
                        _dominated_upper_bound = _bound
                        break
                        ...
                        if len(_hy_components) >= 2:
                            ...
                            if _hy_result.structure_id not in _hy_seen_structure_ids:
                                ...
                                if _hy_result.executable:
                                    ...
                                    _best_found = max(_best_found, _real_marginal)
                ...
                if _dominated_remaining:
                    # A single aggregate row per (anchor, component
                    # subset) -- not one row per unexamined index tuple,
                    # which would be an absurd number of rows for a
                    # mathematical non-result. Honestly distinguishes two
                    # different situations, never conflating them:
                    #   - DOMINATED_WITH_PROOF: the search actually
                    #     reached the point where the next candidate's own
                    #     upper bound could no longer exceed an already-
                    #     achieved real total -- a genuine mathematical
                    #     proof that every remaining combination is worse.
                    #   - SEARCH_DEPTH_LIMIT_REACHED: the documented
                    #     _HYBRID_BB_MAX_EXAMINED_PER_SUBSET ceiling was
                    #     hit first -- the remainder is UNEXAMINED, not
                    #     proven dominated, and this is disclosed as such
                    #     rather than mislabeled.
                    _dom_status = "SEARCH_DEPTH_LIMIT_REACHED" if _search_depth_limit_hit else "DOMINATED_WITH_PROOF"
                    _dom_structure_id = uuid.uuid4()
                    session.add(ProductionStructure(
                        id=_dom_structure_id, project_id=project.id,
                        name=(
                            f"{_anchor_code} + {'/'.join(_subset)} hybrid search "
                            f"({_dominated_remaining} combinations {'unexamined (search budget)' if _search_depth_limit_hit else 'proven dominated'})"
                        ),
                        description=(
                            f"Branch-and-bound over {_anchor_code}'s {'/'.join(_subset)} routing search "
                            + (
                                f"reached its documented {_HYBRID_BB_MAX_EXAMINED_PER_SUBSET}-candidate search "
                                f"depth limit with {_dominated_remaining} combination(s) still unexamined -- "
                                f"the best real total found within budget (${_best_found:,.2f}) is still served "
                                "and ranked normally, but optimality across the full space is not proven."
                                if _search_depth_limit_hit else
                                f"proved {_dominated_remaining} remaining combination(s) cannot exceed the best "
                                f"real total already found (${_best_found:,.2f}); their own upper bound "
                                f"(${_dominated_upper_bound:,.2f}) is provably lower."
                            )
                        ),
                    ...
                    session.add(StructureCalculationResult(
                        ...
                        calculation_trace_json={
                            ...
                            "is_baseline": False, "relocation_cost_normalized": False, "is_directly_comparable": False,
                            "anchor_jurisdiction": _anchor_code, "anchor_program": _anchor_program_slug,
                            "unexamined_combination_count": _dominated_remaining,
                            "best_real_total_found_usd": (
                                None if _best_found == float("-inf") else round(_best_found, 2)
                            ),
                            "proven_upper_bound_of_remainder_usd": (
                                None if _dominated_upper_bound is None else round(_dominated_upper_bound, 2)
                            ),
                            "reason": (
                                f"Documented search-depth limit ({_HYBRID_BB_MAX_EXAMINED_PER_SUBSET} candidates) "
                                "reached before dominance could be proven; remainder unexamined, not dropped."
                                if _search_depth_limit_hit else
                                "Every remaining candidate's own independent-value upper bound is provably "
                                "at or below an already-achieved, fully-priced, legal real total."
                            ),
                        },
                        input_fingerprint=fingerprint,
                    ))
