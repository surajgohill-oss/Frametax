...
def _compute_fingerprint(
    ...
    payload = {
        ...
        # PROJECT_UI_DATA_INTEGRITY correction (2026-09-21), P0 FX
        # fingerprint purity: the P0-FX-001 fix below (superseded by this
        # one) hashed inputs.fx_context -- itself rebuilt, on every call,
        # from production_normalization's mutable, PROCESS-GLOBAL live FX
        # state (FX_RATE_SNAPSHOTS/FX_LIVE_SNAPSHOT_DATE), refreshed by
        # ensure_fx_freshness() from inside GET /projects/{id}/state -- a
        # READ endpoint. Confirmed live: opening one project's Workspace
        # page (which refreshes the shared global for DISPLAY) changed a
        # DIFFERENT, untouched project's own reuse fingerprint the next
        # time IT was opened, and repeatedly reopening the SAME project
        # produced a new fingerprint (and a full re-evaluation) with no
        # real economic input ever changing. A canonical evaluation
        # identity must never depend on a live/mutable global a mere page
        # view can advance -- "GET/read endpoints never mutate canonical
        # evaluation identity" and "opening or refreshing any project
        # cannot change another project's fingerprint" are now hard
        # invariants. FX rates that genuinely feed pricing (native-
        # currency cap conversion, allocation_pricing.py's
        # convert_incentive_cap_to_usd/_fx_native_amount) still use the
        # SAME live-refreshed context at calculation time -- the actual
        # rate used for THIS run is recorded verbatim, per priced
        # candidate, in that candidate's own persisted calculation_
        # trace_json (fx_basis) -- a genuine, immutable, per-result
        # snapshot, exactly the "persist the exact FX snapshot/rates used
        # by the evaluation" this correction requires, just never fed
        # into the reuse-identity check itself. A deliberate, EXPLICIT
        # re-evaluation ("Begin Evaluation"/"Re-run Evaluation") always
        # re-prices against whatever FX context is live at that moment
        # regardless of this fingerprint's own contents, so a genuinely
        # new day's rate is never silently unreachable -- it simply no
        # longer forces itself onto an otherwise-unchanged project on a
        # passive page view.
        #
        # 1.92.0's `fx_context_digest`/`fx_live_snapshot_date` fields are
        # REMOVED (not replaced) from this identity -- never re-add a
        # live-global-derived field here under any name.
        ...
        # PROJECT_UI_DATA_INTEGRITY correction: the raw live-global
        # "fx_live_snapshot_date" field that used to live here (Overview
        # FX Strip Freshness Architecture) is REMOVED -- see the long
        # comment above fx_context_digest's own former location for the
        # full reasoning. Canonical identity never reads production_
        # normalization's mutable module-level FX state, directly or
        # indirectly, under any field name.
    }
...
class _BulkEvaluationWriter:
    ...
    def _route(self, structure, result) -> None:
        ...
        # GD-2 (Globe data contract remediation, 2026-09-20, operator
        # directive): classification is computed ONCE here, at this
        # centralized call site every candidate already passes through
        # exactly once, and STAMPED onto calculation_trace_json before the
        # candidate is persisted -- so it is part of the canonical
        # PERSISTED contract (retained rows and aggregate representatives
        # alike), never solely serve-time projection logic. This same
        # value is what the retention family lane (`family=` below) and
        # aggregation's per-family dominator reconciliation
        # (candidate_group_identity's "structural_classification" field)
        # consume, so retention, aggregation and the served view all
        # resolve to the exact same canonical family value.
        trace["structural_classification"] = classify_structure(trace, result.structure_type, status == STATUS_PRICED)
        result.calculation_trace_json = trace
        ...
        if status == STATUS_PRICED and npc is not None:
            ...
            held = _Held(
                ...
                baseline=is_baseline, family=trace["structural_classification"],
            )
            self._generation.setdefault("input_fingerprint", result.input_fingerprint)
            self._generation.setdefault("engine_version", result.engine_version)
    ...
    async def _persist_generation_summary(self) -> None:
        ...
        batch: list[dict] = []
        for row in self._aggregator.rows(self._retention.dominating_by_type, self._retention.dominating_by_family):
    ...
    if len(_combined_components) >= 2 and _mc_all_partner_candidates:
        ...
        for _mc_comp_a, _mc_comp_b in itertools.combinations(
            ...
            async def _mc_try_combination(_idx: tuple[int, int, int], _current_best: float) -> float | None:
                ...
                # each segment's selected incentive summed over
                ...
                # SegmentEconomics carries no selected_incentive_usd field
                # (only the structure-level pricing result does); a segment's
                # selected incentive is the pricing kernel's own per-segment
                # rule (allocation_pricing.py, the sum that defines
                # AllocationPricing.selected_incentive_usd): the FLOOR when the
                # segment's ceiling requires confirmation, else the ceiling.
                # canonical-1.91.0 fixes an AttributeError here that fired
                # whenever a multi-component combination actually priced.
                _mc_real_marginal = sum(
                    (s.incentive_floor_usd if s.ceiling_requires_confirmation else s.incentive_ceiling_usd)
                    for s in _mc_pricing.segments
