...
class CanonicalProductionStateBuilder:
    ...
    async def _apply_budget(self, state: CanonicalProductionState, project_id) -> None:
        ...
        if not docs:
            # Fresh Project Source-Document Ingestion: the shared retroactive
            # routing trigger (also used by the live Evaluate path via
            # canonical_project_economics.build_project_economic_inputs) --
            # reuses material_routing._route_budget unchanged for a project
            # whose budget Document/DocumentVersion predates commit-time
            # routing. Never a second parsing engine.
            from app.services.material_routing import ensure_current_budget_routed
            ingested = await ensure_current_budget_routed(self.session, project_id)
