...
**Canonical entry point:** `app/services/canonical_evaluation.py::evaluate_project()`. `ENGINE_VERSION = "canonical-1.79.0"`, `STACKING_RULES_VERSION = "1.4.0"` (`app/optimization/stacking_rules.py`).

**Eight-control closeout pass (2026-09-17, `canonical-1.78.0`/`1.79.0`), resumed from `ddfceb3e`:** REG-5 (cost-pool-aware same-jurisdiction pricing) and REG-4 (pure pairwise co-production, no third movable component) are now genuinely `evaluate_project()`-verified. HO-003/HO-007 gained a real capability confirmation (the existing 3-way treaty+component mechanism needs zero new code once real facts are supplied) but their own literal named program combination is not yet reproduced -- a new `MULTI_PRINCIPAL_PARTIALLY_RESOLVED` status, not a false "verified" claim. HO-012 (no IE-FR treaty) and HO-013 (needs 2 simultaneous movable components, unsupported) are confirmed genuine architectural blockers. HO-010/HO-011 have a materially more precise finding (a real, jurisdiction-matched conditional-program catalog exists but under mismatched or absent identities) but remain unwired. See `docs/validation/CLAUDE_STRUCTURAL_STACKING_RUNTIME_CLOSEOUT.md`'s own `CLAUDE_GENERIC_STRUCTURAL_DISCOVERY_EIGHT_CONTROL_CLOSEOUT` section and `docs/validation/CLAUDE_GENERIC_DISCOVERY_19_CONTROL_RECONCILIATION.csv` for full per-control evidence. 13/19 controls now canonically verified (up from 11/19); 6 remain open, each with a specific, real, evidence-based next step.
