...
**Canonical entry point:** `app/services/canonical_evaluation.py::evaluate_project()`. `ENGINE_VERSION = "canonical-1.74.0"`, `STACKING_RULES_VERSION = "1.4.0"` (`app/optimization/stacking_rules.py`).
...
**Not attempted this pass, and not a quick follow-on**: a full per-line canonical fact model (role, citizenship/residency, payee type, look-through amount, payment status, related-party status) applied consistently across every executable labour-based program. This is a new data-model capability spanning most of the pricing engine (qualification underlies every priced program in the system), requires real per-program statutory research with citations before any schema change (the same standard just applied to the one rule fixed here), and risks changing the priced value of a large fraction of the existing, currently-passing test suite if done -- it needs to be its own reviewed workstream, not a rider on this one. Recorded here as `CANONICAL_LABOUR_BASIS_FACT_MODEL` above.
