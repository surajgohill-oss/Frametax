...
**Canonical entry point:** `app/services/canonical_evaluation.py::evaluate_project()`. `ENGINE_VERSION = "canonical-1.75.0"`, `STACKING_RULES_VERSION = "1.4.0"` (`app/optimization/stacking_rules.py`).
...
**Remaining gap, not yet root-caused this pass**: HO-004/HO-005/HO-006's full 3-program combination (federal CPTC/PSTC + Ontario/BC provincial program + OCASE) still does not appear as a tried candidate in the same-jurisdiction group-stack mechanism (`canonical_evaluation.py`'s `location_groups`/`price_program_group_stack` loop), even though CPTC/PSTC now correctly resolve individually and their real 2-way combinations (e.g. `ca_federal_cptc`+`on_ofttc`) now correctly price or correctly reject on the new mutual-exclusivity rule. OCASE itself DOES appear as a `priced_by_code["CA-ON"]` full-relocation candidate, so it should in principle enter the same location group -- why the specific 3-way subset is never tried was not diagnosed this pass (a pre-existing property of the same-jurisdiction group-stack mechanism, not something this pass's labour-basis change touched). Needs direct instrumentation of `location_groups`/`seen_combos` construction, not a fixture-side fix.

**Not attempted this pass, per explicit scope boundary**: a per-line fact model for any program other than CPTC/PSTC, and HO-009/REG-5/multi-principal/grant-overlay work (all explicitly out of scope for this pass).
