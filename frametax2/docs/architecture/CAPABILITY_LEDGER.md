...
1. **Promotion of the 58 quarantined programs** — each requires retaining the administering authority's own current source per calculation-driving proposition. This is a bounded **database-maintenance** task, explicitly *not* a gate blocker (`PROJECT_RULES.md` §3: "zero partially supported programs remain priceable... does not mean fabricating positive verification for every program"). The fail-closed quarantine means shipping in this state is safe rather than merely tolerable.
