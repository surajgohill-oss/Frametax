...
1. **CBA-010 remaining 61-program backfill** — a genuine, disclosed, non-fabricated residual requiring fresh primary-source research per program (already checked against every existing repository artifact this session — DoctrineRecords, SourceProvenance, program_requirements, cultural_point_tables, coverage registry, and every citation's full text — with nothing further recoverable without new research), not an oversight or an unwillingness to continue.
