...
1. **CBA-010 remaining 69-program backfill** — see above; a genuine, disclosed, non-fabricated residual, not an oversight.
2. **The full 35-item runtime acceptance matrix, individually walked with a typed evidence tag per item** — items 1-8 (qualification), the Canada control, LU/FVD reruns, and no-runtime-web are directly covered by tests re-executed this pass and the pass before; Ontario/NY exhaustive controls (items 12-13, partially 16) are newly covered above; items 9-11 (fact ontology beyond CBA-004's existing coverage), several structuring items (17-23), contingency items beyond LU's own (24-26, 31), and durability items 32/34 were not freshly, individually re-executed with a recorded evidence type this pass.
3. **Structuring-opportunity P0/P1 regression** — not re-run as a dedicated batch this pass (the full test suite, which includes `test_structuring_opportunity_patterns.py`, passed at 4425/0/1, which is regression evidence but not the item-by-item walk of all 10 named patterns Section 11 asks for).
4. **Final push** — not performed. The final gate genuinely has not been reached; pushing an intermediate state would misrepresent it as final.
