...
1. **CBA-002** — Codex found 64 conditions across 15 unhandled `RateCondition.kind` values that the rate resolver carries as disclosure text but never executes (e.g. CPTC's real `rate_base_narrower_than_qpe` condition: 25% should apply to a capped "qualified Canadian labour" base, not total QPE — currently a disclosed, deliberately conservative approximation per the doctrine's own comment, "understates, never overstates"). A full typed condition-kind executor is a genuinely large, cross-cutting change touching potentially many of the 71 programs; judged too large a surface to implement correctly and safely within this pass without real risk of silent regression across the served universe. Not attempted.
