...
1. **CBA-010 remaining 61 partial records** — correctly termed `PROVENANCE_INCOMPLETE_EXISTING_RECORD`, **not** "unresearched" (per Section 7's own terminology correction): prior project work genuinely researched these programs; what is absent is the retained primary source, not the underlying rule. Every one was checked against every existing repository artifact across three passes — DoctrineRecords, SourceProvenance, `program_requirements`, `cultural_point_tables`, coverage registry, and each citation's full text — with nothing further recoverable from existing project knowledge. Per Section 7 this does **not** block the consolidation gate; it is a future **database-maintenance** item, and no web research was performed this pass.
2. **The full 35-item runtime acceptance matrix, individually walked with a typed evidence tag per item** — items 1-8 (qualification), the Canada control, LU/FVD reruns (now re-confirmed a third time this pass with an explicit $0 delta), Ontario/NY exhaustive controls plus the new multi-program invariant, and no-runtime-web are directly covered by tests genuinely re-executed across this and the prior passes; a structuring-opportunity spot-check (component/PDV, treaty wiring, all 5 Gemini patterns, 23 tests) passed clean. Items 9-11 (fact ontology beyond CBA-004's existing coverage), several structuring items (17-23) beyond the spot-check above, contingency items beyond LU's own (24-26, 31), and durability items 32/34 were not freshly, individually re-executed with a recorded evidence type this pass.
...
`CANONICAL_BACKEND_AND_GLOBAL_STRUCTURING_INTELLIGENCE_ACCEPTED` / `CANONICAL_BACKEND_RECONNECTION_ACCEPTED` is NOT claimed. This is genuine, substantial, honestly-reported partial progress toward that gate, including a real EXISTS_BUT_DISCONNECTED capability found and reconnected this pass (NY Production Plus) and a new permanent regression guard against the failure class that motivated this phase (the multi-program jurisdiction invariant).
