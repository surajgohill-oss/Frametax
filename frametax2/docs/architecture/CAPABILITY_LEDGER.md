...
**Deferred, unchanged**: the automatic stale-budget-reparse trigger for an EXISTING `BudgetDocument` is deliberately NOT wired into any live call path (see the incident above — this is a genuine, disclosed design decision, not an oversight: silently reprocessing a project's real financial classification on every Evaluate is exactly the kind of un-reviewable economics change this task's own incident proved dangerous); `classify_budget_line_items.py`'s contingency-detection regex does not tolerate the real "Contigency" misspelling (a genuine, generic gap, disclosed, deliberately not fixed per the product owner's explicit "no further LU economics work" direction); FVD's title-page personnel extraction gap (disclosed, not fixed, separate closed capability); Little Utopia's screenplay remains scan-blocked (pre-existing, would require OCR work, out of scope).
