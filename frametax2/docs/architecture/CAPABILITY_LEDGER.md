...
1. **Structured provenance for the 35 remaining programs** — each still requires retaining the administering authority's own current source. This is disclosed, non-blocking database maintenance (their economics already price), not a gate blocker.
2. **The full 35-item runtime acceptance matrix, individually walked with a typed evidence tag per item** — qualification items 1-8, the Canada control, LU/FVD reruns (re-confirmed with an explicit $0 delta across three consecutive passes), Ontario/NY exhaustive controls, the multi-program invariant, and no-runtime-web are all directly covered by tests genuinely re-executed; a structuring-opportunity spot-check (component/PDV, treaty wiring, all 5 Gemini patterns) passed clean. Several items (fact ontology beyond CBA-004, structuring items 17-23 beyond the spot-check, contingency items beyond LU's own, durability items 32/34) were not freshly, individually re-executed with a recorded evidence type.
3. **UI Inspector / sidebar closeout** and **Script Analyzer / three-level Budget Estimator** — unchanged from every prior report, still explicitly PRESERVED/deferred.
