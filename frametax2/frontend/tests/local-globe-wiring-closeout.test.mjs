...
test("buildCountryHoverData's baseIncentive/segmentIncentiveUsd are populated (not null) for a hybrid structure via the component_allocations fallback", () => {
  ...
  const view = buildGlobeView(
    { structures: [s], ranking: [], canonical_selected_structure_id: null, best_per_jurisdiction: {}, top_by_structural_family: {}, optimizer_candidates: [s], optimizer_scenarios: [s], producer_optimizer_options: [s] },
    rankById,
...
test("buildOptimizerPathway's points/arcs carry real qpeUsd from component_allocations, not null for every leg", async () => {
  ...
  const s = hybridStructure();
  const pathway = buildOptimizerPathway({ structures: [s], optimizer_candidates: [s], optimizer_scenarios: [s], producer_optimizer_options: [s] }, "hy-1");
