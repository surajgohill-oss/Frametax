...
// ── Fallback for non-SoFi venues ──────────────────────────────────────────────
function VenueFallbackCards({ venueName }: { venueName?: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <Map size={28} className="text-slate-700 mb-3" />
      <p className="text-sm font-medium text-slate-400 mb-1">
        {venueName ? `${venueName}` : "Venue"} Intelligence
      </p>
      <p className="text-xs text-slate-600 max-w-xs">
        Section-level intelligence is currently available for SoFi Stadium events.
        Additional venues will be added as coverage expands.
      </p>
    </div>
  );
}
...
export default function VenueIntelligence({ eventId, venueSlug, venueName }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [intel, cls] = await Promise.all([
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // if compute fails, still try to reload what we have
      await loadData();
    } finally {
      setRefreshing(false);
    }
  ...
  // ── Non-SoFi fallback ────────────────────────────────────────────────────
  if (!isSoFi) {
    return <VenueFallbackCards venueName={venueName} />;
  }
  ...
  // ── No data ──────────────────────────────────────────────────────────────
  if (!intelligence || !classifications) return null;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mobile: section detail appears below map when selected */}
      {selectedSection && (
      ...
      {/* Mobile: opportunity board always shown below map */}
      {!selectedSection && (
      ...
      <p className="text-[10px] text-slate-700 text-center">
        {intelligence.sections_total} canonical sections · Tap a section for detail
      </p>
    </div>
  );
}
