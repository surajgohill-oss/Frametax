...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.events.listings(id, 8).then(setListings).catch(() => {}),
    ]);
  }, [id]);
  ...
  function renderOverview() {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Forensic: lowest live listings */}
        {listings && listings.length > 0 && (
          <div>
            <h3 className="text-xs text-slate-500 uppercase tracking-wider mb-2">Lowest Live Listings</h3>
            <div className="rounded-xl border border-white/7 bg-[#161b27] overflow-x-auto">
              <table className="w-full text-xs min-w-[480px]">
                <thead>
                  <tr className="border-b border-white/5">
                    {["#", "Price", "Section", "Row", "Qty", "Marketplace"].map((h) => (
                ...
                <tbody>
                  {listings.slice(0, 8).map((l, i) => (
                    <tr key={l.id} className="border-b border-white/4 last:border-0 hover:bg-white/2">
                      ...
                      <td className="px-3 py-2 font-semibold text-slate-100 tabular-nums">{fmt$$(l.price)}</td>
                      <td className="px-3 py-2 text-slate-300 max-w-[140px] truncate">{l.section_name ?? l.section ?? "—"}</td>
                      <td className="px-3 py-2 text-slate-400">{l.row ?? "—"}</td>
                      <td className="px-3 py-2 text-slate-500 tabular-nums">{l.quantity}</td>
                      <td className="px-3 py-2">
                        <span className="text-[10px] text-slate-400 capitalize">{l.marketplace_slug}</span>
                        {l.listing_url && (
                          <a href={l.listing_url} target="_blank" rel="noopener noreferrer"
                             className="ml-2 text-[10px] text-blue-500 hover:text-blue-400">↗</a>
                        )}
                      </td>
                    </tr>
                  ))}
